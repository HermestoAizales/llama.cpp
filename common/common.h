...
struct common_params {
    ...
    float   hisa_sparsity          = 0.5f;  // fraction of blocks to select (0.0=all, 0.9=top 10%)
    bool    hisa_sink_protect      = true;  // protect attention sink tokens from eviction
    ...
    int32_t moe_max_vram_mb      = 0;      // max VRAM for expert weight ring buffer (0 = auto)
    int32_t n_cpu_moe            = 0;      // number of MoE layers to keep on CPU (0 = all on GPU)
    ...
    std::set<std::string> model_tags;      // model tags (informational, not used for routing)
    std::string model_preset = "";  // path to INI preset file (generated by llama-optimizer)              // NOLINT
